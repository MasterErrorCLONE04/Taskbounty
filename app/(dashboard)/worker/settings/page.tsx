...
export default async function WorkerSettingsPage() {
    ...
    const { data: profile } = await supabase
        .from('users')
        .select('name, bio, skills, avatar_url, is_verified, verified_until')
        .eq('id', user.id)
        .single();
    ...
    return (
        <div className="p-12 max-w-4xl mx-auto">
            ...
            <header className="flex justify-between items-start mb-16">
                <div>
                    <h1 className="text-4xl font-black text-slate-900 mb-2 tracking-tight text-shadow-sm">Configuración Profesional</h1>
                    <p className="text-slate-400 font-medium tracking-tight">Administra tu perfil público, tus habilidades y seguridad.</p>
                </div>
            ...
            <div className="space-y-12">
                ...
                <section className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div className="bg-white border border-slate-100 p-8 rounded-[2.5rem] shadow-sm flex items-center gap-6 group hover:border-amber-100 transition-all cursor-pointer">
                        ...
                        <div>
                            <h4 className="font-black text-slate-900 text-sm uppercase tracking-tight">Notificaciones de Bounties</h4>
