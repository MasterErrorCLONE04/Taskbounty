...
export default function DisputeForm({ taskId, onSuccess }: DisputeFormProps) {
    ...
    const handleConfirmDispute = async () => {
        ...
        try {
            ...
            toast(err.message || 'Error al abrir la disputa.', 'error')
        } finally {
            setLoading(false)
        }
    }
    ...
    return (
        <>
            ...
            <ConfirmModal
                ...
                onConfirmAction={handleConfirmDispute}
                title="¿Abrir una disputa?"
                description="Un mediador revisará el caso y su decisión será final. Asegúrate de adjuntar toda la evidencia necesaria."
                confirmText="Sí, abrir disputa"
                cancelText="Volver"
                variant="warning"
                isLoading={loading}
            />
